...
from federatedml.nn.jzf_homo_nn.nn_model import NNModel, DataConverter
...
class KerasSequenceData(tf.keras.utils.Sequence):
    ...
    def __init__(self, data_instances, batch_size, create_label=False):
        ...
        if create_label:  # for a specific LSTM task
            Logger.info(f"create_label is True")
            self.x_shape = (self.x_shape[0] - 1,)
            ...
            for k, inst in data_instances.collect():
                self._keys.append(k)
                self.x[index] = inst.features[:-1]
                self.y[index] = inst.features[1:]
                index += 1
        else:
            num_label = len(data_instances.map(lambda x, y: [x, {y.label}]).reduce(lambda x, y: x | y))
            Logger.info(f"create_label is False: {num_label}")
            if num_label == 2:
                self.y_shape = (1,)
                self.x = np.zeros((self.size, *self.x_shape))
                self.y = np.zeros((self.size, *self.y_shape))
    ...
    def __getitem__(self, index):
        ...
        if self.create_label:  # means lstm, multiple-to-multiple
            return self.x[start: end], self.y[start: end][:, :, None]
        else:
