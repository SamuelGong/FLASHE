        ...
        if (siteLocalAddress == null) {
            ...
            try {
                ...
                for (NetworkInterface ni : Collections.list(networkInterfaces)) {
                    Enumeration<InetAddress> inetAddresses = ni.getInetAddresses();
                    for (InetAddress ia : Collections.list(inetAddresses)) {
			String temp_address = StringUtils.substringAfterLast(ia.toString(), "/");
			// my rules: 172.32.0.0/16 is still my local address
			boolean my_local = temp_address.contains("172.32.");
                        if (my_local) {
                            siteLocalAddress = StringUtils.substringAfterLast(ia.toString(), "/");
                        }
                    }
